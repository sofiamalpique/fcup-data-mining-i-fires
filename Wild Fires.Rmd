...
##### We have a duplicate value: "Viana do Castelo" and "Viana Do Castelo", so we are keeping only one of them: "Viana do Castelo".

### Removing duplicate

```{r}
fires_train <- fires_train %>% mutate(district=replace(district,
...
##### We decided to keep the rows with NA in extinction_time and firstInterv_time at least for now.
...
#### Collect the data with the maximum temperature for each district
...
station_data <- station_data %>% add_row(nearest_district="Viseu",id="PO000008575")
station_data <- station_data %>% add_row(nearest_district="Aveiro",id="PO000008575")
station_data <- station_data %>% add_row(nearest_district="Braga",id="PO000008575")
station_data <- station_data %>% add_row(nearest_district="Viana do Castelo",id="PO000008575")
station_data <- station_data %>% add_row(nearest_district="Vila Real", id="PO000008575")
station_data <- station_data %>% add_row(nearest_district="Guarda", id="POM00008570")
station_data <- station_data %>% add_row(nearest_district="Leiria", id="PO000008535")
station_data <- station_data %>% add_row(nearest_district="Santarém", id="PO000008535")
station_data <- station_data %>% add_row(nearest_district="Portalegre", id="POM00008558")
station_data <- station_data %>% add_row(nearest_district="Setúbal", id="PO000008535")
...
<br/> ####\# Observe if there are any NA that came with the merge
...
ggplot(final_data, aes(x=as.ITime(alert_hour))) + geom_histogram(binwidth = 3600) + 
  ...
  ggtitle("Relationship between alert time and intentional cause")
```

### What can we say about the "origin" and the "intentional_cause"?

```{r}
final_data %>% group_by(origin,intentional_cause) %>% count() %>% arrange(desc(n))
```
...
ggplot(final_data, aes(x=tmax)) + geom_histogram(binwidth = 2) + 
  ...
  ggtitle("Relationship between maximum temperature, origin and intentional cause")
```
